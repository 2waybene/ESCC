...
png ("x:/project2019/RNAseqProj/results/DE/human_K70MD_DGE_adjp05.png")
par(mfrow=c(1,1))
...
write.csv(res[(which(res$padj <= 0.05 )),], file = "x:/project2019/RNAseqProj/results/DE/mouse_DGE_adjp05.csv")

png ("x:/project2019/RNAseqProj/results/DE/mouse_DGE_adjp05.png")
par(mfrow=c(1,1))
# Make a basic volcano plot
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Mouse RNAseq Volcano plot", xlim=c(-3,3)))
# Add colored points: blue if padj<0.01, red if log2FC>1 and padj<0.05)
with(subset(res, padj<.05 ), points(log2FoldChange, -log10(pvalue), pch=20, col="blue"))
with(subset(res, padj<.05 & abs(log2FoldChange)>2), points(log2FoldChange, -log10(pvalue), pch=20, col="red"))
mtext ("TR_SH vs. TR_AG")
dev.off()
